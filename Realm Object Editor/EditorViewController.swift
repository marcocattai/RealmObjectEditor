...
class EditorViewController: NSViewController, NSTableViewDelegate, NSTableViewDataSource, ExtendedTableViewDelegate ,NSTextFieldDelegate, AttributeTypeCellDelegate, RelationshipDestinationCellDelegate, EntityNameCellDelegate, AttributeNameCellDelegate, RelationshipNameCellDelegate, NSUserNotificationCenterDelegate {
    ...
    func entityNameDidChange(_ entity: EntityDescriptor!, newName: String)
    {
        ...
        if row > -1{
            ...
            entitiesTable.reloadData(forRowIndexes: indexSet, columnIndexes: IndexSet(integersIn: NSMakeRange(0, 1).toRange()!))
    ...
    func populateRelationshipUI()
    {
        ...
        if row > -1{
            let indexSet = IndexSet(integer: row)
            relationshipsTable.reloadData(forRowIndexes: indexSet, columnIndexes: IndexSet(integersIn: NSMakeRange(0, 2).toRange()!))
    ...
    func reloadAndReselectAttribute()
    {
        ...
        if row > -1{
            let indexSet = IndexSet(integer: row)
            attributesTable.reloadData(forRowIndexes: indexSet, columnIndexes: IndexSet(integersIn: NSMakeRange(0, 2).toRange()!))
    ...
    {
        let cell = entitiesTable.make(withIdentifier: "entityCell", owner: self) as! EntityCell
    ...
    func cellForAttributeAtRow(_ row: Int, column: NSTableColumn?) -> NSView
    {
        ...
        if column?.identifier == "name"{
            let cell = attributesTable.make(withIdentifier: "name", owner: self) as! AttributeNameCell
            cell.attribute = attribute
            cell.delegate = self
            return cell
        }else{
            let cell = attributesTable.make(withIdentifier: "type", owner: self) as! AttributeTypeCell
            cell.attribute = attribute
            cell.delegate = self
            return cell
        }
    ...
    func cellForRelationshipAtRow(_ row: Int, column: NSTableColumn?) -> NSView
    {
        ...
        if column?.identifier == "name"{
            let cell = relationshipsTable.make(withIdentifier: "name", owner: self) as! RelationshipNameCell
            cell.relationship = relationship
            cell.delegate = self
            return cell
        }else{
            let cell = relationshipsTable.make(withIdentifier: "destination", owner: self) as! RelationshipDestinationCell
    ...
    //MARK: - Export
    func exportToSwift(_ sender: AnyObject!)
    {
        tryToExportWithLangName("Swift")
    }
    
    func exportToObjectiveC(_ sender: AnyObject!)
    {
        tryToExportWithLangName("ObjC")
    }
    
    func exportToAndroidJava(_ sender: AnyObject!)
    {
    ...
    func exportToAndroidKotlin(_ sender: AnyObject!)
    {
